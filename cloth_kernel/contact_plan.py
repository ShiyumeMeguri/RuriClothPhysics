...
CONTACT_LINKS_PER_TEAM_LIMIT = 2

CONTACT_LINKS_PER_TEAM_REASON = (
    "the expander walks any number of links, the static contact capacities do not: "
    "_compute_self_capacities budgets one paired term per team, Sigma se times the largest se "
    "and the two point triangle counterparts, so a team that declares more than the diagonal "
    "and one partner enumerates more pairs than the contact buffers hold; raising this limit "
    "means multiplying those paired terms by the same number")
...
def _accepted_targets(team, source, targets):
    accepted = []
    diagonal_seen = False
    for target in targets:
        slot = int(target)
        if slot == source:
            ...
            continue
        if slot <= 0 or not team["valid"][slot] or not team["enabled"][slot]:
            continue
        accepted.append(slot)
    return accepted
...
def build_tasks(team, team_count, contact_links):
    frame_mask = frame_team_mask(team, team_count)
    use_point = np.zeros(team_count, np.uint8)
    use_edge = np.zeros(team_count, np.uint8)
    use_triangle = np.zeros(team_count, np.uint8)
    contact = []
    intersect = []
    for source in np.flatnonzero(frame_mask):
        slot = int(source)
        for target in _accepted_targets(team, slot, contact_links.get(slot, ())):
            if target == slot:

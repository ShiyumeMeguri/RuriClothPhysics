...
class TopologySnapshot:
    def __init__(self):
        ...
        self.use_connect = None
        self.rest_world_external = {}
...
class SetupTables:
    def __init__(self):
        ...
        self.aim_child = None
        self.aim_rest_reach = None
        self.publish_transform = None
        self.external_transform_rows = {}
...
def _basis_is_reflection(m):
    return bool(np.linalg.det(np.asarray(m, dtype=np.float64)[:3, :3]) < 0.0)


AXIS_NORM_REASON = (
    "the three axis lengths come out of one norm over the columns rather than three norms "
    "over one column each, because these run once per collider per frame and the cost of a "
    "numpy call on a three vector is the call and not the arithmetic; the guard against a "
    "zero length axis is kept as a divisor of one, which leaves such an axis exactly where "
    "the per axis form left it")


def _column_lengths(basis):
    return np.linalg.norm(basis, axis=0)


def _matrix_to_quat(m):
    r = np.array(m, dtype=np.float64)[:3, :3]
    lengths = _column_lengths(r)
    r /= np.where(lengths > 1e-30, lengths, 1.0)
    if _basis_is_reflection(r):
        r = -r
...
def _matrix_scale(m):
    return _column_lengths(np.asarray(m, dtype=np.float64)[:3, :3])


COMPONENT_BASIS_DECOMPOSITION_REASON = (
    "the linear part of a component world matrix is a rotation times a diagonal scale, and "
    "the pair is not unique: the same matrix is also the rotation turned half a turn about "
    "any basis axis times the same scale with the other two axes negated, so four of the "
    "eight per axis sign patterns rebuild it exactly and the axis that was mirrored is not "
    "in the matrix to be read out; what is in the matrix is the length of each basis axis "
    "and one bit, the sign of the determinant, so that is what this returns and nothing "
    "downstream may ask for more; the rotation is proper because a quaternion cannot carry "
    "a reflection at all, which is obtained by negating all three axes when the determinant "
    "is negative, the one choice that does not name an axis, and it is also the choice "
    "Blender itself makes, measured on mathutils Matrix.decompose which reports the scale "
    "of a mirrored object as negative on all three axes")


def decompose_component_basis(m):
    world = np.asarray(m, dtype=np.float64)
    return _matrix_to_quat(world), _matrix_scale(world), _basis_is_reflection(world)
        ...
        for i in range(len(tri_list) - 1):
            ...
            for j in range(i + 1, len(tri_list)):
                ...
                if abs(math.degrees(angle)) > max_angle:
                    continue
                s, t, _, _ = host_math.closest_pt_segment_segment(
...
def _build(snapshot):
    ...
    world_normals = host_math.quat_rotate(
        world_rotations32, np.broadcast_to(host_math.VEC_UP, (n, 3)).astype(np.float32))
    world_tangents = host_math.quat_rotate(
        world_rotations32, np.broadcast_to(host_math.VEC_FORWARD, (n, 3)).astype(np.float32))
    local_normals = host_math.normalize(
        np.einsum('ij,nj->ni', w2l[:3, :3].astype(np.float32), world_normals))
    local_tangents = host_math.normalize(
        np.einsum('ij,nj->ni', w2l[:3, :3].astype(np.float32), world_tangents))
    ...
    if triangles:
        ...
        tri_normals = [np.asarray(host_math.triangle_normal(
        ...
        lv = host_math.normalize(lv, fallback=np.broadcast_to(host_math.VEC_UP, lv.shape))
    ...
    normal_adjustment = host_math.quat_identity((n,))
    if not setup.is_spring and snapshot.normal_alignment_mode != 'NONE':
        ...
        for i in range(n):
            ...
            old_rot = host_math.to_rotation(ln[None].astype(np.float32),
                                            lt[None].astype(np.float32))[0]
            ...
            new_rot = host_math.to_rotation(nor[None].astype(np.float32),
                                            lt[None].astype(np.float32))[0]
            normal_adjustment[i] = host_math.quat_mul(host_math.quat_inverse(old_rot[None]),
                                                      new_rot[None])[0]
    setup.normal_adjustment_rotations = normal_adjustment.astype(np.float32)

    init_inverse_rotation = host_math.quat_inverse(setup.init_rotation.astype(np.float32)[None])[0]
    vertex_rotations = host_math.to_rotation(local_normals, local_tangents)
    transform_local_rotations = host_math.quat_mul(
        np.broadcast_to(init_inverse_rotation, (n, 4)).astype(np.float32), world_rotations32)
    setup.vertex_to_transform_rotations = host_math.quat_mul(
        host_math.quat_inverse(vertex_rotations),
        transform_local_rotations).astype(np.float32)
    setup.vertex_bind_pose_rotations = host_math.quat_inverse(vertex_rotations).astype(np.float32)

    vertex_local_positions = np.zeros((n, 3), dtype=np.float32)
    vertex_local_rotations = host_math.quat_identity((n,))
    for v in setup.baseline_data:
        p = parent_index[v]
        if p < 0:
            continue
        p_rot = vertex_rotations[p]
        inv_p = host_math.quat_inverse(p_rot[None])[0]
        lpos = host_math.quat_rotate(
            inv_p[None], (local_positions[v] - local_positions[p])[None])[0]
        lrot = host_math.quat_mul(inv_p[None], vertex_rotations[v][None])[0]
    ...
    if len(fixed_list) > 0:
        ...
        for i in fixed_list:
            q = host_math.quat_mul(world_rotations32[i][None],
                                   setup.vertex_bind_pose_rotations[i][None])[0]
            nor_sum = nor_sum + host_math.quat_to_normal(q[None])[0]
            tan_sum = tan_sum + host_math.quat_to_tangent(q[None])[0]
        if np.linalg.norm(nor_sum) > 1e-30 and np.linalg.norm(tan_sum) > 1e-30:
            rot = host_math.to_rotation(
                (nor_sum / np.linalg.norm(nor_sum))[None].astype(np.float32),
                (tan_sum / np.linalg.norm(tan_sum))[None].astype(np.float32))[0]
            setup.init_local_gravity_direction = host_math.quat_rotate(
                host_math.quat_inverse(rot[None]), gravity_direction[None])[0].astype(np.float32)
    else:
    ...
    external_rows = {}
    for i, parent_name in enumerate(snapshot.external_parent):
        if not parent_name:
            continue
        row = external_rows.get(parent_name)
        if row is None:
            parent_rest = np.asarray(snapshot.rest_world_external[parent_name], dtype=np.float64)
            row = len(transform_names)
            transform_names.append(parent_name)
            bind_pose = np.concatenate(
                [bind_pose, (np.linalg.inv(parent_rest) @ matrix_world)[None]], axis=0)
            external_rows[parent_name] = row
        skin_indices[i, 0] = row
        skin_weights[i, 0] = 1.0
        skin_indices[i, 1:] = 0
        skin_weights[i, 1:] = 0.0
    setup.external_transform_rows = dict(external_rows)
    ...
    aim_child = np.full(n, -1, dtype=np.int32)
    aim_rest_reach = np.zeros(n, dtype=np.float32)
    connected = snapshot.use_connect
    if connected is not None:
        for child in range(n):
            parent = int(parent_index[child])
            if parent < 0 or not connected[child] or aim_child[parent] >= 0:
                continue
            aim_child[parent] = child
            aim_rest_reach[parent] = np.linalg.norm(
                local_positions[child] - local_positions[parent])
    setup.aim_child = aim_child
    setup.aim_rest_reach = aim_rest_reach
    setup.publish_transform = np.full(n, -1, dtype=np.int32)
    ...
    if len(triangles) > 0:
        ...
        for edge, tri_list in edge_to_triangles.items():
            ...
            for i in range(len(tri_list) - 1):
                ...
                n1 = host_math.triangle_normal(p1[None], p2[None], p3[None])[0]
                attr3 = attributes[e3]
                for j in range(i + 1, len(tri_list)):
                    e4 = _remaining_vertex(triangles[tri_list[j]], edge)
                    p4 = local_positions[e4]
                    attr4 = attributes[e4]
                    n2 = host_math.triangle_normal(p1[None], p2[None], p4[None])[0]

...
def _reference_collider(config, empty):
    if config is None:
        return
    uid = collider_binding.bind(empty)
    for reference in config.collider_collision.collider_references:
        if reference.collider_uid == uid:
            return
    with selection_sync.suppressed():
        config.collider_collision.collider_references.add().collider_uid = uid
...
class RCP_OT_collider_create(bpy.types.Operator):
    ...
    shape: EnumProperty(name="形状", items=properties.COLLIDER_EMPTY_SHAPE_ITEMS,
                        default='SPHERE')
...
CONVERT_SELECTION_REASON = (
    "the button sits in two places and the active object means a different thing in each: "
    "in an object's own physics tab the object is the active one, and in the armature's "
    "cloth panel the active one is the armature, which can never be a collider; an operator "
    "that only looked at the active object was therefore dead in the panel a user actually "
    "sets colliders up from, so it takes the selection, which is what every other collider "
    "operator on that panel already takes")


def _convertible_targets(context):
    targets = []
    for obj in list(context.selected_objects) + [context.object]:
        if collider_geom.can_be_collider(obj) and obj not in targets:
            targets.append(obj)
    return targets
...
class RCP_OT_collider_convert(bpy.types.Operator):
    ...
    bl_label = "设为碰撞体"
    bl_description = ("把选中的空物体或网格物体标记为碰撞体; 网格物体会被设成网格碰撞体, "
                      "并加入当前配置的碰撞体引用列表")
    bl_options = {'REGISTER', 'UNDO'}

    @classmethod
    def poll(cls, context):
        return bool(_convertible_targets(context))

    def execute(self, context):
        config = _active_config(context)
        targets = _convertible_targets(context)
        depsgraph = context.evaluated_depsgraph_get()
        notices = []
        for obj in targets:
            if obj.type != collider_geom.MESH_OBJECT_TYPE:
                continue
            refusal, notice = collider_geom.mesh_collider_reading(obj, depsgraph)
            if refusal is not None:
                self.report({'ERROR'}, refusal.splitlines()[-1])
                return {'CANCELLED'}
            if notice is not None:
                notices.append(notice)
        converted = 0
        for obj in targets:
            collider_geom.mark(obj)
            _reference_collider(config, obj)
            converted += 1
        self.report({'INFO'}, "已设为碰撞体: %d 个" % converted)
        for notice in notices:
            self.report({'WARNING'}, notice.splitlines()[-1])
        return {'FINISHED'}
    ...
    def execute(self, context):
        ...
        for config in _active_settings(context).configs:
            ...
            for index in range(len(references) - 1, -1, -1):
                target = collider_binding.resolve(references[index].collider_uid)
                if not collider_geom.is_collider(target):
...
IMMEDIATE_RESTORE_REASON = (
    "asking the solver to reset is a note it reads on the next frame it solves, so pressing "
    "it while the timeline is standing still looks like a button that does nothing at all: "
    "the pose on the bones right now is the last frame's output and nobody is going to "
    "overwrite it until time moves again; what this does instead is give the pose back here "
    "and now, to every bone the armature has rather than only the ones some configuration "
    "currently names, because the bones a configuration has stopped naming are exactly the "
    "ones nothing else will ever hand back")


class RCP_OT_pose_restore(bpy.types.Operator):
    bl_idname = "ruri_cloth_physics.pose_restore"
    bl_label = "恢复全部变换"
    bl_description = "把本骨架每一根骨骼的姿态变换清回原位, 当场生效; " \
                     "模拟正在跑的话下一帧会从这个姿势重新起算"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        context.view_layer.update()
        self.report({'INFO'}, "已恢复 %d 根骨骼的变换" % len(obj.pose.bones))
        return {'FINISHED'}


class RCP_OT_configs_clear(bpy.types.Operator):
    bl_idname = "ruri_cloth_physics.configs_clear"
    bl_label = "清空骨骼配置"
    bl_description = "删除本骨架上的全部布料配置, 把它们驱动过的骨骼交还回原位, " \
                     "并删掉它们用到的碰撞体空物体(挂在本骨架碰撞体集合下的一并删)"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        count = len(settings.configs)
        runtime.release_driven_pose(context.scene)
        removed = config_io.remove_owned_colliders(settings, context.scene, obj)
        settings.configs.clear()
        settings.active_config_index = 0
        runtime.clear_registry()
        obj.update_tag(refresh={'DATA'})
        context.view_layer.update()
        self.report({'INFO'}, "已清空 %d 条配置, 删掉 %d 个碰撞体" % (count, removed))
        return {'FINISHED'}
...
class RCP_MT_bones(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.separator()
        layout.operator("ruri_cloth_physics.pose_restore", icon='FILE_REFRESH')

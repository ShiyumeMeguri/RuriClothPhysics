...
def _display_update(self=None, context=None):
    from . import runtime
    scene = getattr(context, "scene", None) if context is not None else None
    runtime.refresh_display_planes(scene if scene is not None else bpy.context.scene)
    _redraw()
...
def _live_update(self, context):
    ...
    if obj is None or obj.type != 'ARMATURE':
        return
    from . import runtime
    runtime.release_armature_pose(obj)


def _compile_target_update(self, context):
    from . import runtime
    runtime.notify_compile_target_changed()
...
def _collider_shape_update(self, context):
    ...
    obj = _owner_object(self)
    if obj is not None and self.is_collider:
        wanted = collider_geom.shape_for_object(obj, self.shape)
        if wanted != self.shape:
            self.shape = wanted
            return
        if obj.type == collider_geom.EMPTY_OBJECT_TYPE:
...
COLLIDER_REFERENCE_PICK_REASON = (
    "the list a config keeps is the place a user goes to say which bodies this cloth hits, "
    "so the picker there offers every object rather than only the ones already marked; "
    "offering only the marked ones made the list unusable for the very first collider of a "
    "scene and unusable for a mesh in particular, because a mesh is never marked by having "
    "been created, it is marked by being told to be one, and the place a user expects to "
    "tell it that is exactly this list; picking an object here therefore marks it, which is "
    "the same thing the convert operator does and is written once in collider_geom so the "
    "two cannot drift. A search field takes anything the scene holds and so cannot lean on "
    "a poll to keep the wrong kind out, which is why the refusal a poll used to make "
    "silently is made here out loud, once, at the moment the object is picked")


def _announce(title, message, icon='ERROR'):
...
def _adopt_collider(target):
    from . import collider_geom
    if not collider_geom.can_be_collider(target):
        _announce("这个物体不能当碰撞体", "只有空物体和网格物体可以: %s 是 %s"
                  % (target.name, target.type))
        return False
    if target.type == collider_geom.MESH_OBJECT_TYPE:
        refusal, notice = collider_geom.mesh_collider_reading(
            target, bpy.context.evaluated_depsgraph_get())
        if refusal is not None:
            _announce("这个网格不能当碰撞体", refusal.splitlines()[-1])
            return False
        if notice is not None and not collider_geom.is_collider(target):
            _announce("这个网格朝内, 布料会被关在里面", notice.splitlines()[-1], icon='INFO')
    collider_geom.mark(target)
    _redraw()
    return True
...
def _collider_reference_index_update(self, context):
    ...
    if not 0 <= index < len(self.collider_references):
        return
    target = collider_binding.resolve(self.collider_references[index].collider_uid)
...
_collider_getter, _collider_setter = collider_binding.make_accessors(
    "collider_uid", _adopt_collider)


class RCPColliderReference(bpy.types.PropertyGroup):
    collider_uid: StringProperty(default="", options={'HIDDEN'})
    collider: StringProperty(
        name="碰撞体", get=_collider_getter, set=_collider_setter,
        description="参与本配置碰撞的碰撞体。搜索框列出场景里的物体, 能当碰撞体的是"
                    "空物体(球/胶囊/平面)和网格物体(网格碰撞体)。"
                    "在这里选中一个还没标记的物体, 会自动把它标记成碰撞体, "
                    "网格物体会被标记成网格碰撞体。"
                    "记下的是碰撞体自己身上的标识而不是指向它的指针, 所以改名不会断, "
                    "而复制骨架也不会把碰撞体一起带走")
...
COLLIDER_SHAPE_ITEMS = (
    ...
    ('MESH', "网格", "网格碰撞体: 碰撞体就是网格物体本身。三角形在绑定时从求值后的网格读一次, "
                     "连同当时的世界缩放一起存进碰撞体自身的坐标系, 之后它和其它形状一样只靠"
                     "位姿运动。网格必须是闭合流形, 否则构建期直接拒绝: 内外符号由最近三角形的"
                     "角加权伪法线给出, 开口网格上这个符号会在破口边缘翻过来。",
     'MESH_ICOSPHERE', 3),
)

COLLIDER_EMPTY_SHAPE_ITEMS = tuple(row for row in COLLIDER_SHAPE_ITEMS if row[0] != 'MESH')
...
class RCPColliderSettings(bpy.types.PropertyGroup):
    ...
    is_global: BoolProperty(
        name="全局", default=False,
        description="本碰撞体对场景里每一条布料配置都生效, 谁都不用去引用它: "
                    "场景的墙面、地面这种所有角色都要挡的东西开这个。"
                    "形状不限, 网格碰撞体照样可以开。"
                    "跟着某个角色走的身体碰撞体不要开 —— 逐条引用只算该角色自己的配置, "
                    "开了全局等于让场上每条配置都去算它",
        update=_collider_update)
...
class RCPGizmoSettings(bpy.types.PropertyGroup):
    always: BoolProperty(name="非选中也显示", default=False, update=_redraw)
    enable: BoolProperty(name="启用覆盖显示", default=False, update=_display_update)
    ztest: BoolProperty(name="深度测试", default=False, update=_redraw)
    position: BoolProperty(name="粒子位置", default=True, update=_redraw)
    axis: BoolProperty(name="粒子轴", default=False, update=_display_update)
    shape: BoolProperty(name="形状(线/三角)", default=False, update=_redraw)
    base_line: BoolProperty(name="基线", default=False, update=_redraw)
    depth: BoolProperty(name="深度着色", default=False, update=_redraw)
    animated_position: BoolProperty(name="动画位置", default=False, update=_display_update)
    animated_axis: BoolProperty(name="动画轴", default=False, update=_display_update)
    animated_shape: BoolProperty(name="动画形状", default=False, update=_display_update)
    inertia_center: BoolProperty(name="惯性中心", default=True, update=_display_update)
...
class RCPObjectSettings(bpy.types.PropertyGroup):
    live: BoolProperty(name="实时模拟", default=False, update=_live_update,
    ...
    show_collision_radius: BoolProperty(
        name="碰撞半径", default=False, update=_display_update,
        description="画出每根模拟骨骼实际参与碰撞的体积, 就是它和碰撞体交互用的那个球/扫掠体; "
                    "需要开启实时模拟才有数据")
    display_scope: EnumProperty(
        name="显示范围", update=_redraw,
        items=(('ACTIVE', "当前配置", "只画当前配置"),
               ('ALL', "全部配置", "画出全部配置, 非当前配置压暗")),
        default='ACTIVE',
        description="上面两个覆盖显示共用这一个范围: 显示骨骼与碰撞半径画的都是配置里的东西, "
                    "要看哪几条配置是同一个问题")
    display_depth: BoolProperty(
        name="深度测试", default=False, update=_redraw,
        description="上面两个覆盖显示共用: 关闭时画在模型前面, 不会被身体挡住")
...
class RCPSceneSettings(bpy.types.PropertyGroup):
    compile_target: EnumProperty(
        name="编译目标", items=cloth_target.TARGET_ITEMS, default=cloth_target.DEFAULT_TARGET,
        description="同一份 Warp kernel 源编译到哪里跑。默认 CUDA; CPU 目标是同一份 kernel 源"
                    "编译成主机代码, 没有显卡时用, 会慢一到两个数量级。"
                    "【切换是安全的: 当前目标显存里的模拟状态会先回读回场景再交接, 不会跳变】",
        update=_compile_target_update)

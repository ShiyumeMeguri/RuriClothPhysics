...
POINTS_HASH_BATCH_REASON = (
    "the control points are read through foreach_get rather than one attribute at a time, "
    "because this runs once per curve per configuration on every frame just to establish "
    "that nothing moved -- forty three calls a frame on this character -- and reading a "
    "point's location and handle through the property system costs eighteen times what "
    "reading the whole plane costs. It cannot be cached against an edit counter instead: "
    "dragging a point in the curve widget does not reach depsgraph_update_post, measured, "
    "so a cache keyed on graph updates would hold a stale curve until something unrelated "
...
def _points_hash(node):
    ...
    count = len(points)
    locations = np.empty(count * 2, dtype=np.float32)
    points.foreach_get("location", locations)
    handles = np.empty(count, dtype=np.int32)
    points.foreach_get("handle_type", handles)
    return count, locations.tobytes(), handles.tobytes()

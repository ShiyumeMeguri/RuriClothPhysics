...
class Layer:
    __slots__ = ("identifier", "order", "poll", "collect", "handles", "planes")

    def __init__(self, identifier, poll, collect, handles=None, planes=None, order=0):
        self.identifier = identifier
        self.order = order
        self.poll = poll
...
PLANE_SCOPE_REASON = (
    "what the solver has to bring back off the device is asked of every registered layer and "
    "not only of the ones a poll says are drawing right now, because the poll answers about "
    "the viewport the mouse is in while the download happens once for the whole frame, and "
    "because a layer that starts drawing between two frames would otherwise read a plane the "
    "frame before it had no reason to fetch; the switches a layer reads to answer this live "
    "in the scene, so the answer is the same for every viewport and is asked with the scene")


def collect_planes(scene):
    found = set()
    for layer in _layers:
        if layer.planes is None:
            continue
        found.update(layer.planes(scene))

...
def _direction_arrow(canvas, origin, direction, color):
    heading = host_math.axis_quaternion(np.asarray(direction, dtype=np.float32)[None])[0]
    profile = (ARROW_PROFILE * ARROW_SCALE).astype(np.float32)
    pairs = []
    for fin in range(ARROW_FINS):
        spin = host_math.quat_from_axis_angle(
            host_math.VEC_FORWARD, np.float32(fin * (2.0 * np.pi / ARROW_FINS)))
        points = host_math.quat_rotate(host_math.quat_mul(heading, spin), profile) + origin
